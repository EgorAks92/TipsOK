...
object Routes {
    ...
    const val PcCompactTipPayment = "pc_compact_tip_payment/{billAmountKopecks}?commandId={commandId}&orderId={orderId}&currency={currency}&operationType={operationType}&rrn={rrn}"
    ...
    fun pcCompactTipPaymentFromPc(amount: BigDecimal, commandId: String?, orderId: String?, currency: String?, operationType: String, rrn: String?): String {
        val normalizedCurrency = currency?.trim()?.uppercase().orEmpty().ifBlank { "RUB" }
        val kopecks = amountToMinorUnits(amount, normalizedCurrency)
        return "pc_compact_tip_payment/$kopecks?commandId=${enc(commandId)}&orderId=${enc(orderId)}&currency=${enc(normalizedCurrency)}&operationType=${enc(operationType)}&rrn=${enc(rrn)}"
    }
