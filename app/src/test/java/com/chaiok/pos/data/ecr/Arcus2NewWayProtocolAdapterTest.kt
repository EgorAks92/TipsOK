...
class Arcus2NewWayProtocolAdapterTest {
    ...
    @Test fun chaiOkJsonPayloadIsNotParsedAsPaymentCommand() {
        val adapter = Arcus2NewWayProtocolAdapter({ Arcus2NewWaySettings() }, logger)
        val payload = "{\"proto\":\"" + "chaiok" + "-ecr" + "\",\"version\":1,\"type\":\"payment\",\"amount\":\"10.00\"}"
        val result = adapter.parseIncoming(Arcus2BinLenCodec.encode(encodeWin1251(payload)))
        assertTrue(result !is EcrParseResult.Command || result.command !is PcEcrCommand.Payment)
    }
